...
class WdfreightMaster(Resource):
    class WdfreightModel(db.Model):
        __tablename__ = 'wdfreightmaster'
    ...
    def validate_data(self, df,validate_flag):
        ...
        subset_of_columns = ['destinationCode', 'warehouseCode', 'transporterCode']
    ...
    def post(self):
        try:
            ...
            if file.filename.endswith('.csv') or file.filename.endswith('.xlsx'):
                ...
                missing_columns = set(WdfreightMaster.WdfreightModel.__table__.columns.keys()) - set(df.columns) - {"id"}
                if missing_columns:
                    missing_columns_message = ", ".join(missing_columns)
                    return make_response({"message": f"{missing_columns_message} is/are missing"},400)
                common_columns = list(set(df.columns) & set(WdfreightMaster.WdfreightModel.__table__.columns.keys()))
                ...
                if not inspector.has_table(WdfreightMaster.WdfreightModel.__tablename__):
                    db.create_all()
                else:
                    db.session.execute(text(f"DELETE FROM {WdfreightMaster.WdfreightModel.__tablename__}"))
                    db.session.commit()
                df.to_sql(WdfreightMaster.WdfreightModel.__tablename__, db.engine, if_exists='append', index=False)
                db.session.commit()
