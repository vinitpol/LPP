...
class PwfreightMaster(Resource):
    class PwfreightModel(db.Model):
        __tablename__ = 'pwfreightmaster'
        id = db.Column(db.Integer, primary_key=True)
        plantCode = db.Column(db.String(length=255), nullable=False)
        warehouseCode = db.Column(db.String(length=255), nullable=False)
        transporterCode = db.Column(db.String(length=255), nullable=False)
        timePeriod = db.Column(db.Integer, nullable=False)
        freightCost = db.Column(db.Float, nullable=False)
        handlingCost = db.Column(db.Float, nullable=False)
    ...
    def validate_data(self, df,validate_flag):
        ...
        subset_of_columns = ['plantCode', 'warehouseCode', 'transporterCode']
    ...
    def post(self):
        try:
            ...
            if file.filename.endswith('.csv') or file.filename.endswith('.xlsx'):
                ...
                missing_columns = set(PwfreightMaster.PwfreightModel.__table__.columns.keys()) - set(df.columns) - {"id"}
                if missing_columns:
                    missing_columns_message = ", ".join(missing_columns)
                    return make_response({"message": f"{missing_columns_message} is/are missing"},400)
                common_columns = list(set(df.columns) & set(PwfreightMaster.PwfreightModel.__table__.columns.keys()))
                ...
                if not inspector.has_table(PwfreightMaster.PwfreightModel.__tablename__):
                    db.create_all()
                else:
                    db.session.execute(text(f"DELETE FROM {PwfreightMaster.PwfreightModel.__tablename__}"))
                    db.session.commit()
                df.to_sql(PwfreightMaster.PwfreightModel.__tablename__, db.engine, if_exists='append', index=False)
                db.session.commit()
