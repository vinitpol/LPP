...
class SupplyMaster(Resource):
    # Define a fixed model for the expected CSV structure
    class SupplyModel(db.Model):
        __tablename__ = 'supplymaster'
    ...
    def post(self):
        try:
            ...
            if file.filename.endswith('.csv') or file.filename.endswith('.xlsx'):
                ...
                missing_columns = set(SupplyMaster.SupplyModel.__table__.columns.keys()) - set(df.columns) - {"id"}
                if missing_columns:
                    missing_columns_message = ", ".join(missing_columns)
                    return make_response({"message": f"{missing_columns_message} is/are missing"},400)
                common_columns = list(set(df.columns) & set(SupplyMaster.SupplyModel.__table__.columns.keys()))
                ...
                if not inspector.has_table(SupplyMaster.SupplyModel.__tablename__):
                    db.create_all()
                else:
                    db.session.execute(text(f"DELETE FROM {SupplyMaster.SupplyModel.__tablename__}"))
                    db.session.commit()
                df.to_sql(SupplyMaster.SupplyModel.__tablename__, db.engine, if_exists='append', index=False)
                db.session.commit()
