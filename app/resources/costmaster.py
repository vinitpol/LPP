...
class CostMaster(Resource):
    
    class CostModel(db.Model):
        __tablename__ = 'costmaster'
        ...
        productionCost = db.Column(db.Float, nullable=False)
        packagingCost = db.Column(db.Float, nullable=False)
    ...
    def post(self):
        try:
            ...
            if file.filename.endswith('.csv') or file.filename.endswith('.xlsx'):
                ...
                missing_columns = set(CostMaster.CostModel.__table__.columns.keys()) - set(df.columns) - {"id"}
                if missing_columns:
                    missing_columns_message = ", ".join(missing_columns)
                    return make_response({"message": f"{missing_columns_message} is/are missing"},400)
                common_columns = list(set(df.columns) & set(CostMaster.CostModel.__table__.columns.keys()))
                ...
                if not inspector.has_table(CostMaster.CostModel.__tablename__):
                    db.create_all()
                else:
                    db.session.execute(text(f"DELETE FROM {CostMaster.CostModel.__tablename__}"))
                    db.session.commit()
                df.to_sql(CostMaster.CostModel.__tablename__, db.engine, if_exists='append', index=False)
                db.session.commit()
