    ...
    def post(self):
        try:
            ...
            if file.filename.endswith('.csv') or file.filename.endswith('.xlsx'):
                ...
                missing_columns = set(PlantMaster.PlantModel.__table__.columns.keys()) - set(df.columns) - {"id"}
                if missing_columns:
                    missing_columns_message = ", ".join(missing_columns)
                    return make_response({"message": f"{missing_columns_message} is/are missing"},400)
                common_columns = list(set(df.columns) & set(PlantMaster.PlantModel.__table__.columns.keys()))
                ...
                if not inspector.has_table(PlantMaster.PlantModel.__tablename__):
                    db.create_all()
                else:
                    db.session.execute(text(f"DELETE FROM {PlantMaster.PlantModel.__tablename__}"))
                    db.session.commit()
                df.to_sql(PlantMaster.PlantModel.__tablename__, db.engine, if_exists='append', index=False)
                db.session.commit()
