...
class WarehouseMaster(Resource):
    class WarehouseModel(db.Model):
        __tablename__ = 'warehousemaster'
        id = db.Column(db.Integer, primary_key=True)
        warehouseCode = db.Column(db.String(length=255), nullable=False)
        warehouseName = db.Column(db.String(length=255), nullable=False)
        warehouseType = db.Column(db.String(length=255), nullable=False)
        storageCapacity = db.Column(db.Integer, nullable=False)
        warehoueThroughput = db.Column(db.Integer, nullable=False)
        warehouseLocation = db.Column(db.String(length=255), nullable=False)
        lattitude = db.Column(db.Float,nullable=False)
        longitude = db.Column(db.Float,nullable=False)
        stateCode = db.Column(db.String(length=255), nullable=False)
    ...
    def validate_data(self, df,validate_flag):
        ...
        subset_of_columns = ['warehouseCode', 'warehouseName', 'warehouseType']
    ...
    # @app.route('/upload/v9', methods=['POST'])
    def post(self):
        try:
            ...
            if file.filename.endswith('.csv') or file.filename.endswith('.xlsx'):
                ...
                missing_columns = set(WarehouseMaster.WarehouseModel.__table__.columns.keys()) - set(df.columns) - {"id"}
                if missing_columns:
                    missing_columns_message = ", ".join(missing_columns)
                    return make_response({"message": f"{missing_columns_message} is/are missing"},400)
                common_columns = list(set(df.columns) & set(WarehouseMaster.WarehouseModel.__table__.columns.keys()))
                ...
                if not inspector.has_table(WarehouseMaster.WarehouseModel.__tablename__):
                    db.create_all()
                else:
                    db.session.execute(text(f"DELETE FROM {WarehouseMaster.WarehouseModel.__tablename__}"))
                    db.session.commit()
                df.to_sql(WarehouseMaster.WarehouseModel.__tablename__, db.engine, if_exists='append', index=False)
                db.session.commit()
