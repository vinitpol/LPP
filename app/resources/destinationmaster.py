...
class DestinationMaster(Resource):
    
    class DestinationModel(db.Model):
        # Define the column types based on the expected data types in the database
        __tablename__ = 'destinationmaster'
        id = db.Column(db.Integer, primary_key=True)
        salesOfficeCode = db.Column(db.String(length=255), nullable=False)
        destinationCode = db.Column(db.String(length=255), nullable=False)
        destinationName = db.Column(db.String(length=225), nullable=False)
        lattitude = db.Column(db.Float, nullable=False)
        longitude = db.Column(db.Float, nullable=False)  # Fix the typo here
    ...
    def validate_data(self, df,validate_flag):
        ...
        subset_of_columns = [ 'destinationCode']
    ...
    def post(self):
        try:
            ...
            if file.filename.endswith('.csv') or file.filename.endswith('.xlsx'):
                ...
                missing_columns = set(DestinationMaster.DestinationModel.__table__.columns.keys()) - set(df.columns) - {"id"}
                if missing_columns:
                    missing_columns_message = ", ".join(missing_columns)
                    return make_response({"message": f"{missing_columns_message} is/are missing"},400)
                common_columns = list(set(df.columns) & set(DestinationMaster.DestinationModel.__table__.columns.keys()))
                ...
                if not inspector.has_table(DestinationMaster.DestinationModel.__tablename__):
                    db.create_all()
                else:
                    db.session.execute(text(f"DELETE FROM {DestinationMaster.DestinationModel.__tablename__}"))
                    db.session.commit()
                df.to_sql(DestinationMaster.DestinationModel.__tablename__, db.engine, if_exists='append', index=False)
                db.session.commit()
