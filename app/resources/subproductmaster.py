...
class SubproductMaster(Resource):
    class SubproductModel(db.Model):
        __tablename__ = 'subproductmaster'
        id = db.Column(db.Integer, primary_key=True)
        subProductCode = db.Column(db.String(length=255),nullable=False)
        productCode = db.Column(db.String(length=255),nullable=False)
        subProductName = db.Column(db.String(length=255),nullable=False)
        sales = db.Column(db.String(length=255),nullable=False)
        packType = db.Column(db.String(length=255),nullable=False)
    ...
    def validate_data(self, df,validate_flag):
        ...
        subset_of_columns = ['subProductCode', 'productCode', 'sales', 'packType']
    ...
    def post(self):
        try:
            ...
            if file.filename.endswith('.csv') or file.filename.endswith('.xlsx'):
                ...
                missing_columns = set(SubproductMaster.SubproductModel.__table__.columns.keys()) - set(df.columns) - {"id"}
                if missing_columns:
                    missing_columns_message = ", ".join(missing_columns)
                    return make_response({"message": f"{missing_columns_message} is/are missing"},400)
                common_columns = list(set(df.columns) & set(SubproductMaster.SubproductModel.__table__.columns.keys()))
                ...
                if not inspector.has_table(SubproductMaster.SubproductModel.__tablename__):
                    db.create_all()
                else:
                    db.session.execute(text(f"DELETE FROM {SubproductMaster.SubproductModel.__tablename__}"))
                    db.session.commit()
                df.to_sql(SubproductMaster.SubproductModel.__tablename__, db.engine, if_exists='append', index=False)
                db.session.commit()
